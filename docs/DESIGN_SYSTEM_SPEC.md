...
110 icons from [Phosphor](https://phosphoricons.com), generated by
`scripts/gen_icons.py` from a local Phosphor checkout into
...
is ~13% smaller than RGBA for the same pixels. 153KB for the set.
...
Worse, the shipped SVGs were **Material Symbols** while the PNGs were
**Phosphor** — two different icon libraries under one filename. Making SVG work
would have silently swapped the artwork. All 102 are deleted and the guard now
fails if an `.svg` reappears.
...
both. Against a 35MB app that is +12%. The current 110 cost 153KB.
...
**Weight naming is inconsistent**, inherited from Material Symbols, and the
base name means opposite things across families:

| pattern | base weight | example |
|---|---|---|
| `X` + `X_fill` | base is regular | `account_circle`, `contacts`, `forum` |
| `X` + `X_border` | **base is fill** | `bookmark`, `favorite`, `star` |
| `X` + `X_filled` | base is regular | `star_filled` |

A systematic regular/fill pairing — Phosphor's own model, and the reason the
mapping already carries a weight per name — needs one convention first.
`X` regular + `X_fill` is the obvious one, and adopting it renames roughly
eight families.
