...
Mode: HeroUI Native default visual and interaction alignment; implementation and acceptance are tracked separately.

## 0. Design Decisions

Section 0.1 is decided. Remaining target and API questions must not silently change that baseline.

### 0.1 Visual Identity - Decided

GearUI adopts the open-source **HeroUI Native default appearance and control
feedback**, including component anatomy, typography, surfaces, geometry,
selection presentation, pressed feedback, and motion. This supersedes both the
Tamagui v5 migration and the earlier iOS-only default. Matching colors alone is
not acceptance. Component structure may change where the reference requires it.

The source is HeroUI Native 1.0.9, commit
`b9fa5410b5fbb875475127386166f4c029e9e73f`. The execution contract is
[HeroUI Native Alignment](HEROUI_NATIVE_ALIGNMENT.md). Its source lock lives in
`tokens/reference/heroui-native.lock.json`.

GearUI retains its Kotlin API conventions, KMP/Kuikly runtime, DTCG token data
pipeline, and application-level theme overrides. A rounded default and a square
preset must use the same component anatomy and state implementation. Presets
may replace typography, geometry, palette, materials, and motion; they do not
replace session, navigation, focus, or overlay ownership.

The supplied HeroUI Pro editor screenshots establish the desired breadth of
customization, not proof that its presets exist in the open-source Native repo.
Do not import paid templates. Glass is an optional material with explicit
platform fallback, not an excuse to change the default or claim Liquid Glass.

The historical status below records previous work only. It is **not** the new
default specification or evidence of HeroUI parity. Where older numerical or
anatomy rules conflict, the new alignment contract takes precedence; unaffected
runtime safety and API-family contracts remain mandatory.

Sample scope: restore its original mobile page layouts and example combinations.
Do not retain the Tamagui-era gallery rewrites or temporary DesignReferencePage.
The existing sample consumes updated GearUI components; theme-token plumbing is
preserved. Historical acceptance records below do not authorize page redesign.

### Historical Tamagui Migration (Superseded; Not HeroUI Acceptance)

The current decision supersedes the iOS-only baseline recorded below: GearUI
independently implements the visual rules of the open-source Tamagui v5 default
configuration. The reference is local commit
`91719c62de5b9380ee2829b5e2c6112f38038306` (`@tamagui/config` 2.7.7).
Neither v5-subtle, the website's custom fonts/themes, nor paid templates are
the default reference. Kotlin APIs and Kuikly runtime ownership remain GearUI's.

Matching means geometry, type, surfaces, pressed/hover/focus/disabled feedback,
and motion trajectories, not merely similar screenshots. Native and Web
reference differences remain explicit. Blur remains an optional GearUI
extension, not evidence of default-reference parity. Existing input-focus,
keyboard, safe-area, and overlay workarounds must not be removed without
behavioral verification.

Implementation status:

- `tokens/controls.tokens.json` is the source for regular control geometry;
  `scripts/generate_tokens.py --check` verifies reproducible Kotlin generation.
- The current DTCG profile supports literal px dimensions, sRGB colors, and
  system-font typography with a numeric line-height multiplier. Logical px
  map to Dp, font dimensions to Sp. Unsupported units, color spaces, fonts,
  letter spacing, and aliases fail explicitly. This is not a full resolver.
- Button and Field size defaults now consume generated geometry. The regular
  height is 44; button horizontal padding is 18, field padding is 16.
- Default semantic shapes map to reference radii 0/5/7/9/16, plus a GearUI
  capsule sentinel and `controlLarge = 10`. Button sizes map to 5/7/9/10;
  Input sizes map to 7/9/10. This is not the full reference radius scale.
- Neutral surface colors and neutral Button normal/hover/pressed/focus-ring
  colors are generated. Button now defaults to the neutral theme. Explicit
  primary/danger/success/warning variants remain GearUI extensions.
- Native and Web typography profiles are generated separately (regular body
  17/22 and 15/23 respectively); semantic role mapping is recorded in the JSON.
- The neutral Button uses instant state feedback, matching the reference's
  default without an explicit animation prop. It does not invent a scale or
  spring animation. Keyboard focus draws a ring without changing layout.
- Input and standalone Textarea use replaceable `InputColors`. Hover changes
  the border; focus restores the ordinary border; keyboard focus adds a 2px
  external ring. Transient states are read at draw time, preserving the native
  input modifier and layout. Disabled input keeps the reference surface/text
  palette. Error and card variants remain explicit GearUI extensions.
- Standalone Textarea uses the regular control radius, 16px horizontal and
  13px vertical padding. Compact AutoResizeTextarea keeps its explicit metrics
  as a GearUI extension, not a default-reference equivalent.
- Remaining field-family mappings, overlay motion drivers, and cross-target
  visual/interaction acceptance are pending.
- No default-reference visual or interaction parity is claimed until comparison
  pages have been checked on the relevant targets.

Acceptance so far (not global completion):

- iOS 26.2 / iPhone 17 Pro: default button activation, disabled/loading callback
  rejection, four control sizes, native input focus and typing, two-line Textarea
  reservation/growth, and light/dark switching were exercised in the sample.
- Android and JS: compilation and shared state/geometry tests only; no visual
  acceptance yet. HarmonyOS runtime acceptance has not been performed.
- Hover and focus priority have unit coverage with a negative mutation check.
  Keyboard-focus-ring rendering and pointer-drag cancellation still require
  platform interaction acceptance. Sample screenshots alone do not prove parity
  with a running reference application.
- The base v5 configuration does not bundle an animation driver. The RN and
  CSS driver presets are distinct; overlay motion must be traced to the actual
  reference driver and component transition before changing GearUI's lifecycle.
  A duration-only substitution is not an accepted motion match.

Theme customization:

- `ThemeSpec(colors = brandColors)` derives neutral button states from that
  brand rather than using the built-in neutral palette.
- `ThemeSpec.buttonColors` overrides the state palette explicitly. Copying a
  built-in ThemeSpec retains its explicit button palette, just like any other
  data-class field; set `buttonColors = null` to re-derive from changed colors.
- `ThemeSpec.inputColors` follows the same rule. `Shapes.controlLarge` defaults
  to `lg` when omitted in a custom shape set; built-in themes supply 10px.
- Existing `Typography` names remain the public semantic API. Numeric defaults
  come from the generated platform profile; the static facade delegates to it.
- `DesignReferencePage` is an internal sample fixture for light/dark buttons,
  disabled/loading callback rejection, and Input. It is not a public gallery page.
- Acceptance uses the normal sample entry and component navigation. Fixtures
  supplement that check; they must not replace the installed sample's entry.
  The sample standardization pass may reorder and simplify examples to teach
  the new defaults, rather than preserve obsolete combinations.
- Show neutral/default controls first, then sizes and interaction states, then
  explicitly labelled GearUI extensions (brand/status themes, form labels).
  Do not make every action PRIMARY or every preview a filled gray card.
- Preview frames use the page background, a subtle border, and shared spacing.
  Component surfaces retain their own palette; the frame must not camouflage
  an input against an identical filled surface. Match control sizes in a row.
- LIGHT is a neutral surface-step extension with the same state feedback,
  not an inverse-text workaround. Raw runtime probes stay out of the main
  component introduction. Preserve actual disabled/loading behavior in demos.
- The checked-in upstream ButtonDemo screenshot does not match the current
  ButtonDemo.tsx (different labels/themes). It is not an authoritative v5
  visual baseline. Use current source/config for values and separate runtime
  comparison before declaring parity.

### Historical Baseline (Superseded by 0.1)
...
GearUI Kit is a general-purpose mobile-first UI kit with one coherent design language. The default appearance follows section 0.1; upstream framework types do not leak into the public Kotlin API.
...
Current public radius mapping during the v5 migration:
