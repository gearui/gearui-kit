...
- **历史**：规则确立时存量 3633 行 / 191 文件，先按文件计数冻结（只可减不可增），
  避免批量机翻毁掉最有价值的那些（Input 的 border 为何不能依赖 focus、浮层宿主为何
  不能带 UMD wrapper、iOS Scene 未 active 时的瞬时 0 为何过滤而非零变化必须立即接受）——
  这些逐批手工翻译、每批编译验证。
- **现状**：存量已清零，护栏转为硬门禁，基线文件已删除；任何中文注释直接 fail。
