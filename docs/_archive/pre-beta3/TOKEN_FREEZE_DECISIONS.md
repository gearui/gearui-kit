...
| Batch 7 | 页面壳/导航族: NavBar / BottomNavBar / Page / RuntimeInsets / SafeArea | 460 | 455 | −5 |
| Batch 8 | Feedback 族: Toast / Snackbar / Notification / Progress / Skeleton / Result / EmptyState / Loading / BackTop | 455 | 357 | −98 |
...
- Batch 11 migrates the highest-risk complex input/selector family (DatePicker/Picker/Calendar/CalendarPopup/TreeSelect/Cascader/Transfer/Rate) — pure color/shape mapping (no Spacing, no `*Light`), conservative: only deprecated-bridge migration, no selector interaction / data-fill / popup-behavior change. CalendarPopup's modal scrim `colors.mask → OverlayDefaults.scrimColor` (overlay host untouched). After this cut the library is fully off the four target bridges; the only remaining lib warnings (24) are the dead/internal `TagColorTokens`/`TabColors`/`OverlayPlacement` self-warnings deferred to Batch 13E. No public API change.
- Batch 10 migrates the complex-data-display + display-primitives + foundation-pool group in four conservative cuts. **10A** clears the `DividerSpecs` bucket (Divider/DividerTokens inline `thickness=0.5dp`), plus Badge (colors only, no structure change) and Text/BasicTextField (note `themeColors.*` receiver variant). **10B** clears the `AvatarSpecs` bucket — Avatar's component body finally migrated (the Batch-2 miss), reading `AvatarSizeTokens.Medium`; plus Image/ImageViewer. **10C** is pure color mapping for Table/Tree/Timeline/Watermark/Swiper/SwipeCell (heavy hardcoding, only deprecated-bridge migration, no design re-layout). **10D** cleans the live `ComponentTokens.kt` legacy pool: its root-`Spacing.spacerN` (Float) inlined to Float literals (no pool reshape, per the no-refactor rule), and Tag mapped (`primaryLight→muted`, `successLight/warningLight/dangerLight→…copy(alpha=0.12f)`). `DividerSpecs`/`AvatarSpecs` objects kept (deletion deferred to Batch 13). The dead `TagColorTokens` self-construction (15 self-warnings) is left untouched for Batch 13E, same as `TabColors`. No input/selector, overlay runtime, NavBar/Page, or API change.
- Batch 9 migrates the navigation/disclosure + Tab family in three cuts. **9A** clears the `TabSpecs` bucket on the library side: `TabTokens` inlines `height=48dp / indicatorHeight=2dp`, `primitives/Tab` reads `TabSizeTokens.Medium`; the `TabSpecs` object itself is **kept** (deletion deferred to Batch 13), and the dead `TabColors`/`TabColorTokens` self-warning (1) is left for Batch 13E. TabRow was already clean. **9B** maps Steps/Stepper/Pagination/Anchor/SegmentedControl (Breadcrumb does not exist). **9C** maps NavigationMenu/Collapse/Tour/Drawer; Drawer's modal scrim `colors.mask → OverlayDefaults.scrimColor` (overlay host/gesture untouched), NavigationMenu was NOT merged into ContextMenu (Menu family stays out of scope). No overlay runtime / Select/Dropdown/ContextMenu / NavBar/Page change, no Badge primitive refactor, no public API change.
- Batch 8 opens the post-RC-0 component-family migration wave (Feedback family, per `_archive/BRIDGE_MIGRATION_BATCH_PLAN.md`). Nine files mapped to semantic tokens with the standard rules. Three behavior-preserving inlines: `inverseSurface→foreground` / `inverseOnSurface→background` (Toast, BackTop — the dark/inverse surfaces stay visually identical), and `successLight/warningLight/dangerLight → success/warning/destructive.copy(alpha = 0.12f)` (Snackbar subtle backgrounds). Loading's modal scrim `colors.mask → OverlayDefaults.scrimColor` (the Batch-5 runtime scrim token; overlay host untouched). Spacing migrated off root `Spacing` (Toast also dropped its `as GearSpacing` alias import). No overlay runtime / Dialog/Popup/Sheet change. No public API change. Bucket impact: Colors/Shapes/root Spacing all −, ComponentSpecs unchanged.
- Batch 7 maps the page-shell / navigation family — the final and highest-risk batch (page shell + SafeArea + RuntimeInsets + nav bars), kept deliberately conservative as pure token mapping. NavBar maps `textPrimary→foreground` (title) and `divider→border` (bottom hairline). BottomNavBar maps `textSecondary/textDisabled→mutedForeground` and `divider→border`; its Badge integration was verified by compile + sample only and the Badge primitive was NOT refactored. Page (`primitives/Page.kt`), RuntimeInsetsBridge, and the whole `runtime/` package were verified already token-clean (0 deprecations) — no code change, SafeArea rules left untouched (no PageScaffold exists; `Page.kt` is the shell). No interaction redesign, no navigation-API semantic change, no Web/Harmony logic added. No public API change.
